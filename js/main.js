...
function getGameSpeed() {
    ...
    var gameSpeed = baseGameSpeed * +!gameData.paused * +isAlive() * timeWarpingSpeed * gameSpeedMultiplier
    return gameSpeed
}
...
function createAllRows(categoryType, tableId) {
    ...
    if (!table) {
        console.error(`Table with ID ${tableId} not found for createAllRows.`);
        return;
    }
...
function updateRequiredRows(data, categoryType) {
    ...
    for (requiredRow of requiredRows) {
        var categoryName = requiredRow.id;
        var nextEntity = null;
        var category = categoryType[categoryName];
...
function updateTaskRows() {
    for (key in gameData.taskData) {
        ...
        const maxXp = task.getMaxXp ? task.getMaxXp() : 1;
        progressFill.style.width = (maxXp > 0 ? (task.xp / maxXp * 100) : 0) + "%";
...
function updateHeaderRows(categories) {
    for (categoryName in categories) {
        ...
        if (!headerRow || !headerRow.classList.contains('headerRow')) continue;
...
function checkSkillSkipped(skill) {
    ...
    if (!row) return false;
    var checkbox = row.querySelector(".skipSkill .checkbox");
...
function updateUI() {
    ...
    hideEntities()
    updateText()
    if (typeof updateAgentTabUI === 'function') {
        updateAgentTabUI();
    }
}
...
// --- Initialize RL Agent (after game data is ready) ---
if (typeof initializeAgent === 'function') {
    initializeAgent();
} else {
    console.error("initializeAgent function not found. Ensure rl_agent.js is loaded correctly.");
}
// ----------------------------------------------------
...
// Function to update the multiplier from the slider
function setGameSpeedMultiplier(value) {
    gameSpeedMultiplier = parseFloat(value);
    // Update the display next to the slider
    const display = document.getElementById("gameSpeedDisplay");
    if (display) {
        display.textContent = `x${gameSpeedMultiplier}`;
    }
}

function initializeRequirements() {
    for (const key in gameData.requirements) {
         gameData.requirements[key].elements = [];
    }
    ...
    for (key in gameData.requirements) {
        var requirement = gameData.requirements[key];
        tempData["requirements"][key] = JSON.parse(JSON.stringify(requirement));
    }
}
...
function updateRequirementElements() {
     console.log("Updating requirement DOM element references...")
     for (const key in gameData.requirements) {
         const requirement = gameData.requirements[key];
         if (!requirement) continue;

         let elements = [];
         if (gameData.taskData[key]) {
             elements = [getTaskElement(key)];
         } else if (gameData.itemData[key]) {
             elements = [getItemElement(key)];
         } else {
              switch (key) {
                  case "The Arcane Association": elements = getElementsByClass("The Arcane Association"); break;
                  case "Dark magic": elements = getElementsByClass("Dark magic"); break;
                   case "Shop":
                   case "Rebirth tab":
                   case "Rebirth note 1":
                   case "Rebirth note 2":
                   case "Rebirth note 3":
                     elements = [];
                     break;
                   case "Evil info": elements = [document.getElementById("evilInfo")]; break;
                   case "Time warping info": elements = [document.getElementById("timeWarping")]; break; 
                   case "Automation": elements = [document.getElementById("automation")]; break;
                   case "Quick task display": elements = [document.getElementById("quickTaskDisplay")]; break;
              }
         }
         requirement.elements = Array.from(elements).filter(el => el != null);
     }
}
updateRequirementElements();
...
if (dashboardTabButton) {
    setTab(dashboardTabButton, "dashboard"); 
}
